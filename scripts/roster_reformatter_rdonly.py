...
def reformat_roster(file_path):
    ...
    # Step 19: Keep only RD and A/L rows
    rows_to_delete = []
    for row in range(last_row, 1, -1):  # Iterate from bottom to top
        if ws.cell(row=row, column=1).value not in ["RD", "A/L"]:
            rows_to_delete.append(row)
    
    for row in rows_to_delete:
        ws.delete_rows(row)
    
    print(f"Step 19: Deleted {len(rows_to_delete)} rows not containing 'RD' or 'A/L'")
    wb.save(file_path)
